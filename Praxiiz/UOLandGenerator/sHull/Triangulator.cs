...
namespace DelaunayTriangulator
{
  ...
  {
    public static double distance2To(this Vertex v1, Vertex v2)
    {
      double dx = v1.x - v2.x;
      double dy = v1.y - v2.y;
      return dx * dx + dy * dy;
    ...
    public static double distanceTo(this Vertex v1, Vertex v2)
    {
      return (double)Math.Sqrt(v1.distance2To(v2));
    }
  }
  ...
  class Triangulator
  {
    ...
    public static List<Edge> GetUniqueEdges(List<DelaunayTriangle> triangles)
    {
      Dictionary<Vertex, List<Edge>> edges = new Dictionary<Vertex, List<Edge>>();

      foreach (DelaunayTriangle triangle in triangles)
      {
        //create candidate edges for the triangle
        Edge[] candidateEdges = new Edge[3];
        candidateEdges[0] = new Edge(triangle.Vertices[0], triangle.Vertices[1]);
        candidateEdges[1] = new Edge(triangle.Vertices[1], triangle.Vertices[2]);
        candidateEdges[2] = new Edge(triangle.Vertices[2], triangle.Vertices[0]);

        for (int i = 0; i < 3; ++i)
        {
          if (!edges.ContainsKey(candidateEdges[i].DelaunayV1))
          {
            List<Edge> tempEdgeList = new List<Edge>();
            tempEdgeList.Add(candidateEdges[i]);
            edges.Add(candidateEdges[i].DelaunayV1, tempEdgeList);
          }
          else
          {
            Edge foundEdge = null;
            foreach (Edge e in edges[candidateEdges[i].DelaunayV1])
            {
              if (candidateEdges[i].DelaunayV2.x == e.DelaunayV2.x && candidateEdges[i].DelaunayV2.y == e.DelaunayV2.y)
              {
                foundEdge = e;
                break;
              }
            ...
            if (foundEdge == null)
            {
              edges[candidateEdges[i].DelaunayV1].Add(candidateEdges[i]);
            }
            else
            {
              candidateEdges[i] = foundEdge;
            }
          }

          triangle.Edges[i] = candidateEdges[i];
        }
      }
